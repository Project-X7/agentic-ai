...
def execute_sql(query: str, db_path: str) -> pd.DataFrame:
    sql = query.strip().removeprefix("```sql").removesuffix("```").strip()
    conn = sqlite3.connect(db_path)
    try:
...
def llm(system: str, user: str, model: str = MODEL) -> str:
    ...
    LiteLLM translates the OpenAI-style messages format to each provider's
    native API.  For Groq, set GROQ_API_KEY in your environment and use
    the "groq/<model_name>" prefix.

    To switch providers, just change MODEL at the top of the file:
        "groq/llama-3.3-70b-versatile"   ← Groq  (current)
...
def ask(
    ...
    if verbose:
        if critique.strip() == "NO_ISSUES":
            ...
            print(final_sql)
            print(sep)
            print("  ✏️   SQL was revised." if changed else "  ✅  SQL unchanged after reflection.")
...
if __name__ == "__main__":
    ...
    questions = [
        ...
        # # Q2 — JOIN + event filtering
        # "Which suppliers have the most cancelled purchase orders? "
        # "Show supplier name and cancellation count, top 5.",

        # # Q3 — date arithmetic + NULL awareness
        # "For each received purchase order, how many days elapsed from creation to receipt? "
        # "Show po_id, supplier name, and days_to_receive. Order by longest first, limit 10.",

        # # Q4 — NOT EXISTS / anti-join
        # "List all received purchase orders that have NOT been paid yet. "
        # "Show po_id, supplier name, and total line item value.",

        # # Q5 — derived current status via window / GROUP BY
        # "What is the average order value per supplier broken down by current PO status? "
        # "(Current status = the latest event_type per PO.) Order by supplier name.",
    ]
